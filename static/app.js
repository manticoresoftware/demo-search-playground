...
const MANTICORE_SEARCH_URL = "localhost:9308/search";
...
async function runSearch(query, signal) {
  ...
  showInspector([result.sql], result.requests);
  updateCounts(result);
...
async function runCompare(query, signal) {
  ...
  showInspector(
    ...
    results.flatMap((result) => result.requests),
  );
...
// The same CALL CHAT as a body for Manticore's JSON /search endpoint.
function chatRequest(message) {
  const chat = { query: message, table: TABLE, model_name: CHAT_MODEL, conversation_uuid: state.conversation || "", vector_field: VECTOR_FIELD };
  return `{"chat": {\n${Object.entries(chat).map(([key, value]) => `  ${JSON.stringify(key)}: ${JSON.stringify(value)}`).join(",\n")}\n}}`;
}
